...
PseudoClientSocket::Send(Ptr<Packet> p, uint32_t flags) {
  ...
  if (m_leftToRead <= 0){
    ...
    Simulator::Schedule(Seconds(m_thinkTimeStream->GetValue()), &PseudoClientSocket::request_page, this);
  }
...
void
PseudoClientSocket::request_page() {
  m_leftToSend = PACKET_PAYLOAD_SIZE;
  Simulator::ScheduleNow(&PseudoClientSocket::NotifyDataRecv, this);
}
...
TypeId RequestHeader::GetInstanceTypeId (void) const { return GetTypeId (); }
uint32_t RequestHeader::GetSerializedSize (void) const { return 4; }
void RequestHeader::Print (std::ostream &os) const { /*TODO*/ }
void RequestHeader::SetRequestSize(uint32_t size){ m_requestSize = size; }
uint32_t RequestHeader::GetRequestSize(void) const { return m_requestSize; }
